...
- **Execution-time live-rate recompute**: staging (`Orders/OrderStagingController.cs` `TryRecomputeToLiveRate`) re-prices the wanted amount to the **live** top immediate rate for the planned offered amount (`wanted = floor(offered * getUnits / giveUnits)`; no whole-lot requirement — a partial lot still buys some wanted; liquidity is validated separately by `TryValidateImmediateStock`) *before* typing — so an order captures a better market (asks for more) or adapts to a worse one, never trading stale analysis amounts. The executor (`Orders/SingleHopExecutionController.cs`) then gates placement on a caller-supplied **rate floor** (min wanted-per-offered): each hop's own analysis-planned rate (`Received/Spent`) less `MaxRateSlippagePercent` — F10 and every F5 hop use the same per-hop floor (`Orders/MultiHopExecutionController.cs` `ComputeHopFloor`). Note the analyzer's routes **do not telescope** (a hop may spend inventory it wasn't handed by the prior hop), so a whole-loop rate-product floor is invalid — each hop is gated against its own plan. Hop audits record planned vs recomputed vs actual.
- **Route analysis modes** (`Routing/CurrencyRouteAnalysis.cs`): when the request's start ≠ target it is an acyclic Start→Target search ranked by target units. When start == target (needs ≥2 hops) it is **cycle mode** — profitable loops back to the start currency (e.g. `Chaos→Scarab→Divine→Chaos`), ranked by net start-currency gain. Net gain is exact: `terminal received + leftover start-currency from hop 1 − StartAmount`; stranded non-start currencies are surfaced separately, not counted. `RequireProfit` (default on for the shipped Chaos→Chaos request) drops non-positive loops, so `RouteFound=false` means no profitable loop exists. **Cycle mode ignores `InventoryBalances`** (flows exactly `StartAmount`, telescoping) — folding owned inventory in would let a loop "buy a little, dump the stockpile" and report that liquidation as profit; acyclic Start→Target still uses inventory (converting owned currency is legitimate there). The single structural enabler is that the target-termination check runs before cycle rejection in `Search`; the graph already carries the needed `Chaos/Divine→X` (competing) and `X→Chaos/Divine` (immediate) edges.
...
6. ~~**Multi-hop route execution**~~ — DONE: `Orders/MultiHopExecutionController.cs` (F5, `AllowMultiHopExecution`) chains the selected route's hops end to end — per hop: execute (drives `SingleHopExecutionController`) → wait for fill (poll `panel.Orders` by `PlayerOrderId`, timeout) → collect proceeds to stash (drives `OrderCollectionController`) → fund the next hop (exchange offers inventory-then-stash). It **owns** the executor + collection ticks during a run (the host gates its standalone `TickSingleHopExecution`/`TickOrderCollection` on `!_multiHopController.IsRunning`) so it consumes each hop's audit itself. Halts the whole chain on any hop fault, unfilled-order timeout, shortfall, or `GoldBudget` breach; per-hop audits + totals are written to `FaustusController_multihop-execution-<League>.json` (`Core/FaustusController.MultiHop.cs`). Requires every single-hop-execution and order-collection permission. Hop resolution for any index is `TryBuildHopContext` (`Core/FaustusController.Orders.cs`), shared with F6/F10.
...
**Required alongside steps 4–6**: competing trades (orders not fulfilled instantly) sit in a waiting period. Viewing listings is DONE: `ExportPlacedOrders` (`Core/FaustusController.Orders.cs`) reads `CurrencyExchangePanel.Orders` and exports pending/completed/canceled orders to `FaustusController_placed-orders-<League>.json`. **Collecting** fulfilled orders is DONE (F11, `AllowOrderCollection`): `Orders/OrderCollectionController.cs` ctrl-right-clicks each Completed order's bought slot (order-element child `<4>`, plus leftover offered slot `<5>` when `OfferedItemStackSize > 0`) into inventory, then ctrl+shift+right-clicks one visible inventory stack of each collected currency to sweep all stacks of that type into stash. It needs the exchange, stash, and inventory all open, and only clicks inventory stacks whose rect clears the exchange window's right edge (`rect.Left >= panel.GetClientRectCache.Right`) — the player must block the covered left ~2 inventory columns; covered currencies are reported, never clicked. Still needed with steps 4–6: wait-on and cancel actions for outstanding orders.
...
## TODO
Set   "StartAmount": 500, and make it a setting with the UI
